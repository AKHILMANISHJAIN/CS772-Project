...
if __name__ == '__main__':
    K = np.load('results/MNIST_Laplace_kernel.npy')
    Jtheta = np.load('results/MNIST_Laplace_gp_predictive_mean.npy')
    Var_y = np.load('results/MNIST_Laplace_predictive_noise.npy')
    Var_f = np.load('results/MNIST_Laplace_predictive_var_f.npy')
    ps = np.load('results/MNIST_Laplace_predictive_mean.npy')
    Ys = np.concatenate([np.repeat([i], 30) for i in range(10)])
    plot_kernel(K, Ys, tab10, 'mnist_laplace_kernel')
    plot_observations(ps, Ys, np.arange(10), tab10, 'mnist_laplace_pred_mean_ste', width=550)
    plot_observations(Var_f, Ys, np.arange(10), tab10, 'mnist_laplace_var_f', width=450,
                      plot_var=True)
    plot_observations(Var_y, Ys, np.arange(10), tab10, 'mnist_laplace_var_y', width=450,
                      plot_var=True)
